...
# Auto-generate workbook if missing
...
def reset_excel():
    ...
    try:
        ...
        if not wb:
            ...
            app = None  # Don't close Excel if already open
        ...
        # Clear Dashboard but preserve headers
...
# === Buttons ===
# ttk.Button(frm_right, text="Run Script", command=run_script, bootstyle=PRIMARY).pack(pady=5, fill=X)
...
# ttk.Button(frm_right, text="Generate Excel Sheet", command=generate_excel, bootstyle=WARNING).pack(pady=5, fill=X)
